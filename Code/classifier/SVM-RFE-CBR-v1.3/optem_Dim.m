function [bestacc,best_dim,ftRank,ftScore,bestmcc,bestDIM]=optem_Dim(XX,label,param)

[ftRank,ftScore] = ftSel_SVMRFECBR(XX,label,param);

cg_str = ['-c ' num2str(param.rfeC) ' -g ' num2str(param.rfeG) ' -b 1 ' '-v 5' '-t 2'];
...
for i=10:2:feature_dim
    ...
    Predict_label=svmpredict(label,XX,model1,'-b 1');
